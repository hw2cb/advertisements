...
{
    public class AdvertisementImagesDriver : ContentFieldDisplayDriver<MediaField>
    {
        ...
        public AdvertisementImagesDriver(AttachedMediaFieldFileService attachedMediaFieldFileService,
        ...
        {
            return Combine(Initialize<DisplayMediaFieldViewModel>(GetDisplayShapeType(context), model =>
            {
                model.Field = field;
        ...
        public override IDisplayResult Edit(MediaField field, BuildFieldEditorContext context)
        {
            ...
            return Combine(Initialize<EditMediaFieldViewModel>("Advertisement__Images__Edit", model =>
            {
            ...
            }).Location("Content:3").OnGroup("User"),

            Initialize<EditMediaFieldViewModel>(GetEditorShapeType(context), model =>
            {
                var settings = context.PartFieldDefinition.GetSettings<MediaFieldSettings>();
                if (settings.AllowMediaText)
                {
